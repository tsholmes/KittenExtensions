...
Custom attributes that are checked before loading the assets (e.g. custom load actions that extend the default KxAssetAttribute) can be added using the `KxCustomAttribute`. The `KxCustomAttribute` must be in the KittenExtensions namespace and have a `static bool Check(Type type, CustomAttributeData attr)` method.\
Other assemblies that then use the custom attribute don't need the check method or the KxCustomAttribute applied to the custom attribute but it needs to be in the same namespace as the original.
...
namespace ShaderExtensions
{
    ...
    internal class SxUniformBufferAttribute(string xmlElement) : Attribute
    {
        public static bool Check(Type type, CustomAttributeData attr)
        {
            ...
            if (!KXProvider.ValidateArg(attr, 0, out string xmlElement, out var err))
                return false;
